...
{
    NSString *itemWeight = @"0元/1元/5元/50元";
    NSArray * firstArray = [itemWeight componentsSeparatedByString:@"/"];
    
    [[pickerView.subviews objectAtIndex:1] setHidden:TRUE];
    [[pickerView.subviews objectAtIndex:2] setHidden:TRUE];
    //    UIView * qview = (UIView *)[pickerView.subviews objectAtIndex:1];
    //    NSLog(@"%@",NSStringFromCGRect(qview.frame));
...
-(void) cancelAction
{
    if ([_delegate respondsToSelector:@selector(premiumCancelAction)]) {
        [_delegate premiumCancelAction];
    }
}
...
- (void) okAction
{
    ...
    if ([_delegate respondsToSelector:@selector(premiumOKAction:)]) {
        [_delegate premiumOKAction:string];
    }
}
