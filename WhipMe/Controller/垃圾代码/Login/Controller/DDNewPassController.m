...
#define NEWPASSWORD @"输入新密码"


@interface DDNewPassController ()<UITextFieldDelegate>
//第一次输入密码
...
@implementation DDNewPassController
...
- (void) initNavigationController
{
    ...
    self.navigationItem.title = NEWPASSWORD;
...
- (void)createTextField
{
    ...
    self.secondPassWordField.placeholder = @"请输入密码(6-16位字符)";
    //当编辑的时候显示清除按钮
...
- (void)achieveBtnClick
{
    ...
    if ([self checkOutTextFieldByZZ]) {
        DDLoginController *loginController = [[DDLoginController alloc]init];
