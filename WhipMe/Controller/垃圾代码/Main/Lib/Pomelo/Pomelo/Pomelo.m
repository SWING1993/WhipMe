...
- (void)processMessage:(NSDictionary *)msg
{
    ...
    if (msgId && msgId > 0){
        ...
        if (callback != nil) {
            NSMutableDictionary *tmpDic = [(NSDictionary *)msg mutableCopy];
            tmpDic = [[tmpDic objectForKey:@"body"] mutableCopy];
            
            for (int i = 0; i < [tmpDic allKeys].count; i ++) {
                NSString *key = [[tmpDic allKeys] objectAtIndex:i];
                id value = [tmpDic objectForKey:key];
                if ([value isKindOfClass:[NSNull class]] || value == NULL || value == nil) {
                    [tmpDic setObject:@"" forKey:key];
                }
            }
            callback(tmpDic);
