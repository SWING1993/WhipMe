...
//    /*平移带缩放效果的动画
    ...
    CGFloat scale = ABS(_baseView.frame.size.height - ABS(xoffset)) / _baseView.frame.size.height;
    CGFloat scale1 = ABS(_baseView.frame.size.height - ABS(xoffset)) / _baseView.frame.size.height;
    NSLog(@"%f",_baseView.frame.size.height);
    if (_baseView.frame.size.height == 736) {
        scale1 = ABS(_baseView.frame.size.height+85 - ABS(xoffset)) / _baseView.frame.size.height;
    } else if (_baseView.frame.size.height == 667) {
        scale1 = ABS(_baseView.frame.size.height+50 - ABS(xoffset)) / _baseView.frame.size.height;
    } else if (_baseView.frame.size.height == 568) {
        scale1 = ABS(_baseView.frame.size.height-20 - ABS(xoffset)) / _baseView.frame.size.height;
    } else if (_baseView.frame.size.height == 480) {
        scale1 = ABS(_baseView.frame.size.height-55 - ABS(xoffset)) / _baseView.frame.size.height;
    }
    ...
    if (xoffset>0) {//向右滑的
        _currentView.transform = CGAffineTransformMakeScale(scale1, scale1);
        [_currentView setFrame:CGRectMake(xoffset, _baseView.bounds.origin.y + (totalHeight * (1 - scale1) / 2), totalWidth * scale, totalHeight * scale1)];
    }else{//向左滑的
        _currentView.transform = CGAffineTransformMakeScale(scale, scale);
