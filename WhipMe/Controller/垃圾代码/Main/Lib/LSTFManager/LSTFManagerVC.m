...
//  LSTFManagerVC
//
//  Created by Steven on 15-3-5.
//  Copyright (c) 2015年 Steven. All rights reserved.
//

#import "LSTFManagerVC.h"
#import "LSTFKeyBoardToolBarView.h"
#define ios6x [[[UIDevice currentDevice] systemVersion] floatValue] < 7.0f

@interface LSTFManagerVC ()<UITextFieldDelegate>
...
//键盘工具条
@property (nonatomic,strong) LSTFKeyBoardToolBarView *keyBoardToolBarView;
...
+(void)installManagerForVC:(UIViewController *)vc scrollView:(UIScrollView *)scrollView tfModels:(NSArray *(^)())tfModels{
    ...
    if(vc==nil){
        NSLog(@"LSTFManagerVC：安装失败，您传入的控制器不合法，不能为nil");
        return;
    }
    ...
    LSTFManagerVC *tfManagerVC=[[LSTFManagerVC alloc] init];
...
+(BOOL)checkTFModels:(NSArray *)tfModels{
    ...
    if(tfModels==nil || tfModels.count==0){
        ...
        NSLog(@"LSTFManagerVC：安装失败，您传入的textField数组不合法，没有数据");
    ...
    for (id obj in tfModels) {
        ...
        NSLog(@"LSTFManagerVC：安装失败，tfModels数组内部的对象不是TFModel模型对象");
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    ...
    LSTFKeyBoardToolBarView *toolBarView = (LSTFKeyBoardToolBarView *)textField.inputAccessoryView;
...
-(LSTFKeyBoardToolBarView *)keyBoardToolBarView{
    
    __weak typeof(self) weakSelf=self;
    
    if(!_keyBoardToolBarView){
        
        _keyBoardToolBarView=[LSTFKeyBoardToolBarView keyBoardToolBarView];
